...
render :: FilePath -> Value -> ActionT LText WebM LText
    ...
    Right template -> do
      ...
      then do
        ...
        case tlResult of
          ...
          Right t  -> do
            flashes <- getFlashes
            pure $ toLText $ substituteValue t $
...
getFlashes :: ActionT LText WebM Text
getFlashes = do
  maybeError <- getError
  maybeWarning <- getWarning
  maybeInfo <- getInfo
  traceShowM maybeInfo
  let err = maybe "" errorTemplate maybeError
  let warning = maybe "" warningTemplate maybeWarning
  let info    = maybe "" infoTemplate maybeInfo
  pure $ err <> warning <> info

infoTemplate :: Text -> Text
infoTemplate msg = [i| <div class="flex bg-green-lighter max-w-sm mb-4">
...
errorTemplate msg = [i| <div class="flex bg-red-lighter max-w-sm mb-4">
...
warningTemplate :: Text -> Text
warningTemplate msg = [i| <div class="flex bg-yellow-lighter max-w-sm mb-4">
